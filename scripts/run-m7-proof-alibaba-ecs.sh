...
readonly CORE="$PROOF_ROOT/scripts/run-m7-proof-core.sh"

readonly INHERITED_EVIDENCE_DIR="${M7_PROOF_EVIDENCE_DIR:-}"
readonly FIXED_EVIDENCE="$PROOF_ROOT/proof-artifacts"
readonly EVIDENCE="$FIXED_EVIDENCE"
export M7_PROOF_EVIDENCE_DIR="$EVIDENCE"
...
guard_run_authorization() {
  local token="${M7_PROOF_RUN_AUTHORIZATION:-}"
  [[ -n "$token" ]] || die "Missing one-shot Human run authorization ($RUN_AUTH_NAMESPACE_DESCRIPTION)"

  local prefix=''
  if [[ "$token" =~ ^M7-EXI-01-RUN-([A-Fa-f0-9]{7,40})-[A-Za-z0-9_-]+$ ]]; then
